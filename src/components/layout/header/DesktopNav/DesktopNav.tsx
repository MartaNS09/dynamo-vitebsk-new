...
import { useState, useRef, useEffect } from "react";
import Link from "next/link";
import { useRouter, usePathname } from "next/navigation"; // ← добавил usePathname
import { NAV_ITEMS } from "@/lib/config/navigation";
import "./DesktopNav.scss";
...
export const DesktopNav: React.FC<DesktopNavProps> = ({ onNavClick }) => {
  ...
  const menuRef = useRef<HTMLDivElement>(null);
  const router = useRouter();
  const pathname = usePathname(); // ← получаем текущий путь здесь
  ...
  useEffect(() => {
    if (showSchoolMenu && menuRef.current) {
            ...
            return (
              ...
              >
                <button
                  ...
                  aria-expanded={showSchoolMenu}
                  aria-haspopup="true"
                  aria-controls="school-submenu"
                  aria-label={`О школе, ${
                    showSchoolMenu ? "меню открыто" : "меню закрыто"
                  }. Нажмите Enter для перехода на главную.`}
                >
                  <span>О школе</span>
                  <svg
                    width="12"
                ...
                {showSchoolMenu && item.submenu && (
                  <div
                    ...
                    role="menu"
                    aria-label="Подменю раздела 'О школе'"
                    tabIndex={-1}
                    onMouseEnter={() => setShowSchoolMenu(true)}
                    onMouseLeave={() => setShowSchoolMenu(false)}
                  >
          ...
          return (
            <li key={item.href} className="desktop-nav__item">
              <Link
                ...
                className="desktop-nav__link"
                onClick={onNavClick}
                aria-current={pathname === item.href ? "page" : undefined} // ← исправлено: pathname вместо router.pathname
              >
                <span>{item.label}</span>
              </Link>
            </li>
